...
def create_formatted_pdf(text_content: str, topic: str) -> str:
    ...
    try:
        ...
        st.error("Missing 'DejaVuSans.ttf' or 'DejaVuSans-Bold.ttf'.")
        return ""
...
def handle_query_logic(query: str, session_id: str = None):
    if session_id:
        ...
        if not os.path.exists(temp_db_path):
            return "Document session expired. Please upload again.", None
        db = FAISS.load_local(temp_db_path, embeddings, allow_dangerous_deserialization=True)
    else:
        if not os.path.exists(FAISS_INDEX_PATH):
            return "Default knowledge base unavailable. Please upload a document.", None
